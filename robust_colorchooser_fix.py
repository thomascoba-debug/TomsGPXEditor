#!/usr/bin/env python3
"""
Robust Color Utils - Add fallback for missing colorchooser

This script adds a robust fallback mechanism for tkinter.colorchooser
that handles cases where colorchooser is not available.
"""

import os

def add_colorchooser_fallback():
    """Add robust fallback for colorchooser"""
    ...
    print("ROBUST COLORCHOOSER FALLBACK")
    print("=" * 50)
    
    # Check if fallback already exists
    if 'HAS_COLORCHOOSER' in content:
        print("Fallback already implemented")
        return False
    
    # Create robust version with fallback
    robust_content = '''"""
Color Utilities
...
'''
    ...
    print("✅ Added robust colorchooser fallback")
    print("✅ File updated with fallback mechanism")
    
    return True

if __name__ == "__main__":
    success = add_colorchooser_fallback()
    
    if success:
        print("\n✅ ROBUST FALLBACK ADDED!")
        print("The colorchooser issue is now handled gracefully.")
        print("If colorchooser is not available, a simple fallback dialog will be used.")
        print("\n🚀 Try running: python main.py")
    else:
        print("\nℹ️  NO CHANGES NEEDED!")
        print("The file already has fallback implemented.")
